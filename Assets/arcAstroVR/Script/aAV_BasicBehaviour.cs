...
public class aAV_BasicBehaviour : MonoBehaviour
{
	...
	private List<aAV_GenericBehaviour> behaviours;            // The list containing all the enabled player behaviours.
	...
	public aAV_ThirdPersonOrbitCamBasic GetCamScript { get { return camScript; } }
	...
	void Awake ()
	{
		...
		behaviours = new List<aAV_GenericBehaviour> ();
		overridingBehaviours = new List<aAV_GenericBehaviour>();
	...
	void FixedUpdate()
	{
		...
		{
			foreach (aAV_GenericBehaviour behaviour in behaviours)
			{
		...
		{
			foreach (aAV_GenericBehaviour behaviour in overridingBehaviours)
			{
				behaviour.LocalFixedUpdate();
			}
		}
	...
	public void SubscribeBehaviour(aAV_GenericBehaviour behaviour)
	{
	...
	public bool OverrideWithBehaviour(aAV_GenericBehaviour behaviour)
	{
		...
		if (!overridingBehaviours.Contains(behaviour))
		{
			...
			if (overridingBehaviours.Count == 0)
			{
				...
				foreach (aAV_GenericBehaviour overriddenBehaviour in behaviours)
				{
	...
	public bool RevokeOverridingBehaviour(aAV_GenericBehaviour behaviour)
	{
		if (overridingBehaviours.Contains(behaviour))
		{
	...
	public bool IsOverriding(aAV_GenericBehaviour behaviour = null)
	{
		if (behaviour == null)
...
//		Debug.DrawLine(this.transform.position + Vector3.up * 2 * colExtents.x, Vector3.down , Color.red);
...
public abstract class aAV_GenericBehaviour : MonoBehaviour
{
	//protected Animator anim;                       // Reference to the Animator component.
	protected int speedFloat;                      // Speed parameter on the Animator.
	protected aAV_BasicBehaviour behaviourManager;     // Reference to the basic behaviour manager.
	...
	{
		// Set up the references.
		behaviourManager = GetComponent<aAV_BasicBehaviour> ();
